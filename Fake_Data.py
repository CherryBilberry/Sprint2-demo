...
#type_lawyers = np.array(['Paralegal','Junior Associate','Senior Associate','Principal Associate','Partner'])
...
# DD_Report
# DD_report should be positively correlated with DD_review
# suppose rev~[a,b], rep~[c,d], rep = (rev-a)/(b-a)*(d-c)+c
# then add noise = an element of fluc * p
...
d = {'value':value,'dd_rev_paralegal':dd_rev_paralegal, 'dd_rev_ja':dd_rev_ja,'dd_rev_sa':dd_rev_sa,
    ...
    'td_partner':td_partner,'tn_paralegal':tn_paralegal,'tn_ja':tn_ja,'tn_sa':tn_sa,'tn_pa':tn_pa,'tn_partner':
    tn_partner,'num_docs':num_docs,'size_comp':size_comp,'industry':industry,'revenue':revenue,'dr':dr,'ts':ts,'ju':ju}
df = pd.DataFrame(d)
...
y_train = df[['dd_rev_paralegal','dd_rev_ja','dd_rev_sa','dd_rev_pa','dd_rev_partner','dd_rep_ja','dd_rep_pa','dd_rep_sa',
              'dd_rep_paralegal','dd_rep_partner','dd_res_paralegal','dd_res_ja','dd_res_sa','dd_res_pa','dd_res_partner',
             'td_paralegal','td_ja','td_sa','td_pa','td_partner','tn_paralegal','tn_ja','tn_sa','tn_pa','tn_partner']][:9999]
y_true = df[['dd_rev_paralegal','dd_rev_ja','dd_rev_sa','dd_rev_pa','dd_rev_partner','dd_rep_ja','dd_rep_pa','dd_rep_sa',
              'dd_rep_paralegal','dd_rep_partner','dd_res_paralegal','dd_res_ja','dd_res_sa','dd_res_pa','dd_res_partner',
             'td_paralegal','td_ja','td_sa','td_pa','td_partner','tn_paralegal','tn_ja','tn_sa','tn_pa','tn_partner']].iloc[9999,:]
